...
test("Episodes renders without errors with empty data.", () => {
    render(<Episodes episodes={[]} />);
});

test("Episodes renders with correct data when passed.", () => {
    render(<Episodes episodes={testEpisodes} />);

    expect(screen.getByText(/Season 1, Episode 1/)).toBeVisible();
    expect(screen.getByText(/Chapter Two/)).toHaveTextContent(/The Weirdo/);
});